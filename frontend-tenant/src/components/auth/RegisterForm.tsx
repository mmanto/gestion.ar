 ...
 * réplica del bloque de la landing ius pero respaldado por el backend real.
 *
 * Es agnóstica a router/contextos a propósito: tanto la página `/registro`
 * del panel como el embed de la landing estática (micro-frontend) la
 * reutilizan, inyectando cada una su propio comportamiento de red vía
 * `submit`/`google`. Así la UI vive en un solo lugar. El redireo post-éxito
 * (pago o dashboard) se hace con `window.location.href` para funcionar
 * igual dentro y fuera de la SPA.
 */
...
interface RegisterFormProps {
  ...
  /** Plan preseleccionado (p.ej. desde `?plan=anual` de la landing). Null → mensual. */
  initialPlan?: RegisterPlan | null;
...
export const RegisterForm: React.FC<RegisterFormProps> = ({ tenantId, branding, initialPlan, submit, google }) => {
  const primaryColor = branding.primaryColor || PRIMARY;
  const logo = branding.logoUrl;
  const tenantName = branding.tenantName || 'Tu cuenta';

  const [plan, setPlan] = useState<RegisterPlan>(initialPlan === 'anual' ? 'anual' : 'mensual');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        const res = err.response; // unknown — narrowed por `in`
        if (res && typeof res === 'object' && 'data' in res) {
          const data = res.data; // unknown — narrowed por `in`
