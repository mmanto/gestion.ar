Assets nativos para el APK Android del tenant `laboralia`.

Antes de poder correr `./stack.prod build-android laboralia ...` hace falta:

1. Completar en `.env.prod`:
   ```
   TENANT_APPID_LABORALIA=laboralia.intellify.pro   # o el paquete que corresponda
   TENANT_APPNAME_LABORALIA=laboralia Staff         # nombre visible del ícono
   TENANT_BRANDCOLOR_LABORALIA=#000000              # color de marca (splash/status bar)
   ```
2. (Opcional, recomendado) Colocar acá `logo.png` — un logo cuadrado de al
   ...
   existente en `sites/laboralia-landing/`). `build-android` lo usa con
...
3. (Opcional) Colocar acá `google-services.json`, descargado desde el
   proyecto de Firebase creado para `laboralia` (Project Settings → General
   → tu app Android, con el mismo `applicationId` que
   `TENANT_APPID_LABORALIA`). Sin este archivo, el APK compila igual pero
   sin push notifications para este tenant.
