...
cmd_build_android() {
  ...
  echo "  [1/3] VITE_API_URL=${api_url} VITE_TENANT_ID=${tenant_id} npm run build:capacitor ..."
  (cd "$project_dir" && VITE_API_URL="$api_url" VITE_TENANT_ID="$tenant_id" npm run build:capacitor) || {
