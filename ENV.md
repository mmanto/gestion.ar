...
| `EMBEDDING_MODEL` | ❌ | Modelo de embeddings RAG: ID de Hugging Face Hub o ruta local a un snapshot descargado. Default: bakeado en la imagen (si el build alcanza huggingface.co); si el host no lo alcanza, apuntar a un snapshot local montado (p. ej. `/models/embedding-model`) | `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2` |
